...
def propose(draft, head_src, ctx, block, position_ids, start, dcache) -> torch.Tensor:
    """Run the drafter once on a masked block anchored at ``start``.

    ctx covers positions [start - ctx_len, start). Returns the block with positions 1..
...
def ar_generate(target, input_ids, max_new_tokens, stop_token_ids) -> GenResult:
    ...
    return GenResult(ids, n, dt, rounds=[1] * len(tokens), target_calls=calls + 1)
...
def three_stage_generate(
    ...
    while not done:
        ...
        res.checks.append({"pending": pending, "accepted": a_t, "window": policy.current_window()})
        policy.checked += min(a_t + 1, pending)
        policy.rejects += int(a_t < pending)

        # drafter context: target features for [d_keep, new_anchor)
        d_keep = min(dlen, new_anchor - 1)
        crop(dcache, d_keep)
        dlen = d_keep
        tfeats = extract_context_feature(tout.hidden_states, draft.target_layer_ids)
        ctx = tfeats[:, d_keep - tf : new_anchor - tf]

        mlen = min(mlen, new_anchor)
        crop(mcache, mlen)
